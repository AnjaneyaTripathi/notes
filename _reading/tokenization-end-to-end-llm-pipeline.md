...
### The full end-to-end pipeline (this is the exact narrative interviewers ask for

— have it fluent) Step 1 — Preprocessing - Raw text cleanup: normalize unicode, handle casing (some models lowercase, some are case-sensitive), strip/handle special characters, sometimes remove or normalize whitespace/HTML artifacts. - For RAG specifically: this is also where chunking happens — splitting long documents into passages that fit the model's context window and make sense as retrievable units (fixed-size chunking, sentence-aware chunking, or semantic chunking based on topic shifts). Step 2 — Tokenization - Text →subword tokens →integer token IDs, using the model's specific tokenizer (each model ships with its own trained tokenizer/vocabulary — you cannot mix tokenizers across model families). - Add special tokens, truncate or pad to the model's max sequence length / context window. Step 3 — Embedding - Token IDs →dense vectors via the embedding matrix lookup (the earlier discussion). At this point, positional information is also added (positional encoding/embeddings) so the model knows token order, since self-attention alone is permutation-invariant and wouldn't otherwise know sequence order. Step 4 — Inference (the model forward pass) - Embedded tokens flow through the transformer's stacked layers of self-attention (each token's representation is updated based on relevanceweighted context from all other tokens) and feed-forward networks, becoming increasingly contextaware representations layer by layer. - For generative models: the final layer produces a probability distribution over the vocabulary for the next token; a decoding strategy picks the actual next token (greedy, beam search, or sampling methods like top-k / top-p / temperature-controlled sampling) — then the process repeats autoregressively, one token at a time, until an end-of-sequence token or max length is hit. - For embedding/encoder-only models: instead of generating tokens, the final hidden states are pooled (mean pooling / CLS token) into a fixed-size vector output (the earlier discussion). Step 5 — Post-processing - Generated token IDs →detokenization back into human-readable text (reversing step 2). - Application-level post-processing: formatting, stripping stop sequences/special tokens, structured-output parsing (e.g., extracting JSON), safety/moderation filtering, citation attachment (in RAG), or business-logic validation before returning the response to the user. One clean, fluent version to say out loud in the interview: “Raw text is first cleaned and, for something like RAG, chunked into passages. It's then tokenized into subwords using the model's tokenizer and converted to token IDs, with special tokens and padding added as needed.
